...
def get_unet(chans=1):
    concat_axis = 3
    inputs = layers.Input(shape=(512, 512, chans))
...
def get_band_data(loc, dates, bands, mean=None, std=None):
    if mean is None:
        mean = [None for _ in bands]
    if std is None:
        std = [None for _ in bands]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--bands', nargs="*", help='List of bands', type=int, required=True)
    ...
    x_train, y_train, mean, std = get_band_data(args.location, [x for i, x in enumerate(dates) if i!=args.validation], args.bands)
    x_test, y_test, _, _ = get_band_data(args.location, [x for i, x in enumerate(dates) if i==args.validation], args.bands, mean, std)
    ...
    with open(f'history_3months_200epochs_8chann_v{args.validation}_{args.location}_s{args.seed}_b{args.band1}.pkl', 'wb') as f:
        pickle.dump(history.history, f)

    model.save(f'model_3months_200epochs_8chann_v{args.validation}_{args.location}_s{args.seed}_b{args.band1}.h5')
