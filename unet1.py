...
def get_band_data(loc, dates, b, mean=None, std=None):
    ...
    x = np.concatenate([np.load(f"X_B{b}_{loc}_{d}.npy") for d in dates], axis=0)
    if mean is None:
        mean = x.mean()
        std = x.std()
...
if __name__ == "__main__":
    ...
    x_train, y_train, mean, std = get_band_data(args.location, [x for i, x in enumerate(dates) if i!=args.validation], args.band1)
    x_test, y_test, _, _ = get_band_data(args.location, [x for i, x in enumerate(dates) if i==args.validation], args.band1, mean, std)
