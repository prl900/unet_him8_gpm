...
def generate_plot(loc, t):
    fig = plt.figure(figsize=(24, 6))

    ax = fig.add_subplot(1, 4, 1, projection= aaea)

    img = np.load(f'/data/GPM_HIM8/models/yhat_{loc}_v0_b8_s1.npy')[t,:,:,0]
    ax.imshow(img, origin='upper', extent=img_extents[loc], cmap=prec_cm, norm=LogNorm(vmin=0.01, vmax=15))
    ...
    ax = fig.add_subplot(1, 4, 2, projection= aaea)

    img = np.load(f'/data/GPM_HIM8/models/yhat_{loc}_v0_b11_s1.npy')[t,:,:,0]
    ax.imshow(img, origin='upper', extent=img_extents[loc], cmap=prec_cm, norm=LogNorm(vmin=0.01, vmax=15))
    ...
    ax = fig.add_subplot(1, 4, 3, projection= aaea)

    img = np.load(f'/data/GPM_HIM8/models/yhat_{loc}_v0_b11_16_s1.npy')[t,:,:,0]
    ax.imshow(img, origin='upper', extent=img_extents[loc], cmap=prec_cm, norm=LogNorm(vmin=0.01, vmax=15))
    ...
    ax = fig.add_subplot(1, 4, 4, projection= aaea)

    img = np.load(f"/data/GPM_HIM8/exp_paper/Y_{loc}_201811.npy")[t,:,:]
    ax.imshow(img, origin='upper', extent=img_extents[loc], cmap=prec_cm, norm=LogNorm(vmin=0.01, vmax=15))
    ...
    plt.savefig(f"{loc}_prcp_cmp_{t:04d}.png")
