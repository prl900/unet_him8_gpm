...
def generate_plot(loc, t):
    ...
    for band in range(9):
        ax = fig.add_subplot(2, 5, band+1, projection= aaea)
        img = np.load(f"/data/GPM_HIM8/exp_paper/X_B{band+8}_SYD_201811.npy")[t,:,:]  
        ax.imshow(img, origin='upper', extent=img_extents["SYD"], cmap=cm.Greys_r)

        ax.set_extent(img_extents["SYD"], crs=aaea)
        ax.coastlines()
    ...
    ax = fig.add_subplot(2, 5, 10, projection= aaea)

    img = np.load(f"/data/GPM_HIM8/exp_paper/Y_SYD_201811.npy")[t,:,:]
    ...
    plt.savefig(f"inout_dataset_{loc}_{t:04d}.png")

    
generate_plot("SYD", 100)
